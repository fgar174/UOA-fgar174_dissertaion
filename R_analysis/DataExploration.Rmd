...
wind_and_tide_data <- read_excel("wind_and_tide_data.xlsx")
```
...
yard_moves_by_year.df_base <- yard_moves_by_year.df_base[order(yard_moves_by_year.df_base$mve_gkey), ]
```
...
ggplot(yard_moves_by_year_unique,
  ...
  labs(
    title = "Distribution of Freight Kind",
    subtitle = "By Years, 2023 and 2024",
    x = "Freight Kind",
    y = "Count (in thousands)"
  ) +  # Update y-axis label
...
ggplot(
  ...
  geom_point(stat = "summary", fun = mean, size = 3) +  # Add points for monthly averages
  labs(
    title = "Average Days in Port Each Month in 2023 by Freight Kind and Category",
    x = "Month",
    y = "Average Days in Port",
    color = "Freight Kind"
  ) +
...
## GENERATES DATABASE FOR THE WHOLE PORT
...
subset_df[subset_df$time_in > as.POSIXct("2023-12-31 23:59:59"), ]$FOR_TEST <- TRUE
subset_df$time_in <- as.Date(as.POSIXct(subset_df$time_in))

subset_df <- inner_join(subset_df, wind_and_tide_data, by = c("time_in" = "DateTime"))
...
ggplot(
  ...
  geom_bar(aes(y = ..count.. / 1000),
           fill = "steelblue",
           color = "white") +
  labs(title = "Number of container that arrived in 2023 per Month", x = "Month", y = "Count (in thousands)") +
...
ggplot(
  ...
  geom_bar(aes(y = ..count.. / 1000),
           fill = "steelblue",
           color = "white") +
  labs(title = "Number of container that arrived in 2024 per Month", x = "Month", y = "Count (in thousands)") +
  scale_x_date(date_labels = "%B", date_breaks = "1 month") +  # Show each month on the x-axis
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
ufv_gkey <- yard_moves_by_ufv_gkey_2024$ufv_gkey[2]
ufv_gkey_data <- yard_moves_by_year.df_base[yard_moves_by_year.df_base$ufv_gkey == ufv_gkey, ]

# write.xlsx(ufv_gkey_data, "filtered_data.xlsx", rowNames = FALSE)

ufv_gkey_data$t_fetch <-
  as.POSIXct(ufv_gkey_data$t_fetch, format = "%Y-%m-%d %H:%M:%S")

ufv_gkey_data$ACUM_DAYS_IN_PORT <- as.numeric(round(
  difftime(ufv_gkey_data$t_fetch, ufv_gkey_data$time_in, units = "days")
))
...
subset(ufv_gkey_data,
       select = c("mve_gkey", "to_pos_stack", "to_pos_row", "to_pos_level"))


yard_moves_by_year.df_base[yard_moves_by_year.df_base$mve_gkey >= min(ufv_gkey_data[!is.na(ufv_gkey_data$mve_gkey), ]$mve_gkey) &
                             yard_moves_by_year.df_base$mve_gkey <= max(ufv_gkey_data[!is.na(ufv_gkey_data$mve_gkey), ]$mve_gkey), ]


yard_moves_by_year.df_base <- yard_moves_by_year.df_base[order(yard_moves_by_year.df_base$mve_gkey), ]


get_previous_levels <- function(df,
                                current_to_pos_level,
                                current_to_pos_row,
                                current_to_pos_stack,
                                min_mve_gkey) {
  # Get the record for the one before if current_to_pos_level >= 2
  lower_level <- tail(df[!is.na(df$mve_gkey) &
                           df$mve_gkey < min_mve_gkey &
                           !is.na(df$to_pos_stack) &
                           df$to_pos_stack == current_to_pos_stack &
                           !is.na(df$to_pos_row) &
                           df$to_pos_row == current_to_pos_row &
                           !is.na(df$to_pos_level) &
                           df$to_pos_level == current_to_pos_level - 1, ], 1)
  
  lower_level_val <- head(df[!is.na(df$mve_gkey) &
                               df$mve_gkey > min_mve_gkey &
                               !is.na(df$fm_pos_stack) &
                               df$fm_pos_stack == current_to_pos_stack &
                               !is.na(df$fm_pos_row) &
                               df$fm_pos_row == current_to_pos_row &
                               !is.na(df$fm_pos_level) &
                               df$fm_pos_level == current_to_pos_level - 1, ], 1)
  
  # Get the record for the one before if current_to_pos_level == 3
  lower_level_2 <- tail(df[!is.na(df$mve_gkey) &
                             df$mve_gkey < min_mve_gkey &
                             !is.na(df$to_pos_stack) &
                             df$to_pos_stack == current_to_pos_stack &
                             !is.na(df$to_pos_row) &
                             df$to_pos_row == current_to_pos_row &
                             !is.na(df$to_pos_level) &
                             df$to_pos_level == current_to_pos_level - 2, ], 1)
  
  lower_level_2_val <- head(df[!is.na(df$mve_gkey) &
                                 df$mve_gkey > min_mve_gkey &
                                 !is.na(df$fm_pos_stack) &
                                 df$fm_pos_stack == current_to_pos_stack &
                                 !is.na(df$fm_pos_row) &
                                 df$fm_pos_row == current_to_pos_row &
                                 !is.na(df$fm_pos_level) &
                                 df$fm_pos_level == current_to_pos_level - 2, ], 1)
  
  # Initialize an empty result table
  result <- data.frame()
  # Check if lower_level and lower_level_val should be returned
  if (nrow(lower_level) > 0 && nrow(lower_level_val) > 0) {
    # Ensure both have matching ufv_gkey_data before appending
    if (!is.na(lower_level$ufv_gkey) &&
        !is.na(lower_level_val$ufv_gkey) &&
        lower_level$ufv_gkey == lower_level_val$ufv_gkey) {
      result <- rbind(result, lower_level, lower_level_val)
    }
  }
  
  # Check if lower_level_2 and lower_level_2_val should be returned
  if (nrow(lower_level_2) > 0 && nrow(lower_level_2_val) > 0) {
    # Ensure both have matching ufv_gkey_data before appending
    if (!is.na(lower_level_2$ufv_gkey) &&
        !is.na(lower_level_2_val$ufv_gkey) &&
        lower_level_2$ufv_gkey == lower_level_2_val$ufv_gkey) {
      result <- rbind(result, lower_level_2, lower_level_2_val)
    }
  }
  
  # Return the union of all valid, matching results
  return(result)
}

combined_result <- data.frame()  # Initialize an empty dataframe to store results

# Loop through each row of the ufv_gkey_data dataframe
for (i in 1:nrow(ufv_gkey_data)) {
  # Extract the necessary values for the current iteration
  current_to_pos_level <- ufv_gkey_data$to_pos_level[i]
  current_to_pos_row <- ufv_gkey_data$to_pos_row[i]
  current_to_pos_stack <- ufv_gkey_data$to_pos_stack[i]
  min_mve_gkey <- ufv_gkey_data$mve_gkey[i]
  
  # Run the get_previous_levels function
  result <- get_previous_levels(
    yard_moves_by_year.df_base,
    current_to_pos_level = current_to_pos_level,
    current_to_pos_row = current_to_pos_row,
    current_to_pos_stack = current_to_pos_stack,
    min_mve_gkey = min_mve_gkey
  )
  
  # Append the result to the combined result dataframe if it's not empty
  if (nrow(result) > 0) {
    combined_result <- rbind(combined_result, result)
  }
}

combined_result <- combined_result[order(combined_result$mve_gkey), ]

# subset_fergusson_df[subset_fergusson_df$time_in > as.POSIXct("2023-12-31 23:59:59"), ]$FOR_TEST <- TRUE
# subset_fergusson_df$time_in <- as.Date(subset_fergusson_df$time_in)
# 
# subset_fergusson_df <- inner_join(subset_fergusson_df,
#                                   wind_and_tide_data,
#                                   by = c("time_in" = "DateTime"))


combined_result$RELATED <- TRUE

# Get the unique values of ufv_gkey_data$to_pos_block
unique_blocks <- unique(ufv_gkey_data[!is.na(ufv_gkey_data$to_pos_block), ]$to_pos_block)

# Get the minimum and maximum values of mve_gkey in ufv_gkey_data
min_mve_gkey <- min(ufv_gkey_data$mve_gkey, na.rm = TRUE)
max_mve_gkey <- max(ufv_gkey_data$mve_gkey, na.rm = TRUE)

ufv_gkey_data <- yard_moves_by_year.df_base[
  yard_moves_by_year.df_base$to_pos_block %in% unique_blocks &
  yard_moves_by_year.df_base$mve_gkey > min_mve_gkey &
  yard_moves_by_year.df_base$mve_gkey < max_mve_gkey &
  !yard_moves_by_year.df_base$mve_gkey %in% combined_result$mve_gkey,
]

ufv_gkey_data$RELATED <- FALSE

ufv_gkey_data <- rbind(ufv_gkey_data, combined_result)

write.csv(
  ufv_gkey_data,
  paste0("../python_modeling/dataset/ufv_gkey_", ufv_gkey, ".csv"),
  row.names = TRUE
)
```
